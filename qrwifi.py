...
ADMIN_PASSWORD = os.getenv("ADMIN_PASSWORD", "qr1234")
...
def load_library():
    """Load the QR code library from file. Create file if it doesn't exist."""
...
def update_wifi_qr(qr_id):
    ...
    try:
        ...
        return jsonify({"success": True, "entry": library[entry_index]})
...
def get_library_entry(qr_id):
    library = load_library()
    entry = next(
        (item for item in library if item.get("qr_id") == qr_id), None)
    if entry:
...
def get_qr_embed(embed_id):
    ...
    library = load_library()
    entry = next((item for item in library if item.get(
        "embed_id") == embed_id), None)
...
def webhook_update(qr_id):
    ...
    entry = next(
        (
            item
            for item in library
            if item.get("qr_id") == qr_id or item.get("embed_id") == qr_id
        ),
        None,
    )
    ...
    return jsonify(
        {
            ...
            "last_modified": (
                os.path.getmtime(LIBRARY_FILE) if os.path.exists(
                    LIBRARY_FILE) else 0
            ),
        }
    )
